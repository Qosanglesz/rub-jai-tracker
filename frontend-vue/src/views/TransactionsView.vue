  ...
  <div class="space-y-6">
    ...
    <div class="bg-white shadow-sm overflow-hidden rounded-xl border border-gray-100">
      ...
      <div v-else-if="transactions.length === 0" class="text-center py-16 px-4">
        ...
        <p class="text-sm text-gray-500 mb-4">No data for the selected month. Start tracking!</p>
    ...
    >
      <form @submit.prevent="submitTransaction" class="space-y-4">
        ...
        <div>
          ...
          <div v-if="categoriesLoading" class="flex items-center gap-2 text-sm text-gray-400 py-2">
            <LoaderIcon class="h-4 w-4 animate-spin" /> Loading...
          </div>
          <select v-else id="category" v-model="form.categoryId" required
          ...
          <p v-if="filteredCategories.length === 0 && !categoriesLoading" class="text-xs text-amber-600 mt-1">
...
import {
  PlusIcon, LoaderIcon, ReceiptIcon, TrendingUpIcon,
  TrendingDownIcon, CalendarIcon, Trash2Icon
...
// ==================== Lifecycle ====================
watch([selectedMonth, selectedYear], fetchTransactions);
onMounted(() => Promise.all([fetchTransactions(), fetchCategories()]));
</script>
